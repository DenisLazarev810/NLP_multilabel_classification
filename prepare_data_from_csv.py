...
# функция принимает ссылку на docx-документ и возвращает документ, разбитый стоп-словами на части, которые позже и загонятся в pandas dataframe
def separate_docx_by_names( file_dir, break_words ):
    # считаем файл
    doc = docx.Document(file_dir)
    fullText = []
    pieces = [] # куски текста, разбитые стоп-словами
    for para in doc.paragraphs:
        ...
        for i in range( len( txt ) ):
            if ( txt[i] == '\n' ):
                j += 1
        print( 'number of lines in par = ', j )
